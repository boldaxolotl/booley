...
- **Native coverage:** explicitly collect Verilator Coverage Campaigns with `booley flow sim --target sim_soc --coverage`, with optional Target-bound thresholds and approved waivers. Then ask the Coverage Analyst to explain the exact saved Campaign. Simulation and coverage verdicts remain independent ([workflow](docs/user/FLOW_REFERENCE.md#native-coverage-campaigns)).
