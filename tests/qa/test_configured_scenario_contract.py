...
def test_picorv32_public_qa_corrections_are_explicitly_contractual():
    ...
    for name in ("wave", "find", "sample", "distance", "value"):
        assert "interactive.wishbone-baseline" in scenario_step(
            scenario, f"bwave.semantic-{name}"
        )["requires"]
    for name in ("list", "signal", "diff", "stats", "stuck"):
        assert "interactive.wishbone-baseline" not in scenario_step(
            scenario, f"bwave.rejection-{name}"
        )["requires"]
    assert "interactive.reproduce" in scenario_step(
        scenario, "interactive.trace-observability"
    )["requires"]
    assert "interactive.trace-observability" in scenario_step(
        scenario, "interactive.bwave-diagnosis"
    )["requires"]
