...
class TestReviewBaseKeyExpansion:
    ...
    @pytest.mark.parametrize("value", [{"target": "sim_uart"}, ["sim_uart"]])
    def test_review_tb_quality_rejects_target_binding(self, value):
        with pytest.raises(ValueError, match=r"source-scoped.*scalar only"):
