...
The e2e runs **inside the Sandbox Image**, which pins cocotb. From the
repo root, with a freshly built image:

```bash
docker run --rm -v "$PWD/tests/fixtures/cocotb_counter":/work -w /work \
  booley-sandbox python3 -m booley.flows.sim --work-dir /work --target <target>
```
...
| G11 timeout | `--target sim_hang --timeout-ms 15000` | never pass; timeout verdict |
| G12 Elaboration Check | `python3 -m booley.flows.sim --work-dir /work --target sim_verilator --mode elab-only` | PASS (build half untouched) |
| G13 dry-run | `--target sim_icarus --dry-run` | one batched command: `fusesoc … --setup && make … && python3 -m booley.flows.sim.backends.cocotb … --test ×3` |
