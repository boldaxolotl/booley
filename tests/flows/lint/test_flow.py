...
class TestLintResolution:
    ...
    def test_prepare_drives_make_over_resolved_build_root(
        ...
        assert captured["vlnv"] == "::lint_demo:0"
...
class TestDoctorTargetAuthority:
    ...
    def _author_target(
        ...
        *,
        ...
        root.joinpath(filename).write_text(
            ...
            "    flow_options:\n"
            "      tool: verible\n"
            f"      booley: {{doctor_selftest: {str(doctor_selftest).lower()}}}\n",
            encoding="utf-8",
        )

    def test_public_lint_rejects_doctor_only_target_before_resolution(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """An ordinary Flow invocation cannot cross Doctor's private seam."""
        self._author_target(
            tmp_path,
            "doctor.core",
            "acme:ip:doctor:1.0",
    ...
    def test_selected_vlnv_pins_qualified_doctor_target_execution(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Lint forwards the selected core identity through both command paths."""
        self._author_target(
            tmp_path,
            "public.core",
            "acme:ip:public:1.0",
            doctor_selftest=False,
        )
        self._author_target(
            tmp_path,
            "doctor.core",
        ...
        resolve.assert_called_once_with(
            ...
            build_root=(
                tmp_path / ".booley_project" / ".runtime" / "edalize" / "lint" / "doctor_lint"
            ),
        ...
        target_index = setup.index("--target")
        assert setup[target_index + 1] == "lint"
        assert setup[-1] == "acme:ip:doctor:1.0"

    def test_selected_doctor_target_keeps_its_lint_parser(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Lint uses the authorized handle after Doctor context is gone."""
        import booley.targets.target as target_selection

        (tmp_path / "doctor.core").write_text(
            "CAPI=2:\n"
            "name: acme:ip:doctor:1.0\n"
            "targets:\n"
            "  lint_selftest_bad:\n"
            "    flow: lint\n"
            "    flow_options:\n"
        ...
        def select_for_doctor(
            ...
            monkeypatch.setattr(
                target_selection,
                "select_target",
                MagicMock(side_effect=AssertionError("Target authority was re-requested")),
            )
            monkeypatch.setattr(
                target_selection,
                "select_targets",
        ...
        monkeypatch.setattr("booley.flows.lint.flow.select_targets", select_for_doctor)
        monkeypatch.setattr(
            LintFlow,
            "_prepare_lint_command",
            lambda _self, _target: (["verible-verilog-lint"], _stub_resolved("verible")),
        )
        monkeypatch.setattr(
            LintFlow,
            "_execute",
            lambda _self, _cmd: MagicMock(
                returncode=0,
                stdout="rtl/bad.sv:1:1: forbidden construct [module-filename]\n",
                stderr="",
                timed_out=False,
                duration_s=0.1,
            ),
...
class TestWarningsOnlyFatalExit:
    ...
    def test_non_blocking_knob_passes_warnings_only_fatal_exit(
        ...
        flow.read_state()
        with patch(
            "booley.flows.lint.flow._lint_warnings_as_errors",
            return_value=False,
        ):
            result = flow._run()
...
class TestErrorVsFailTaxonomy:
    ...
    def test_hard_fail_error_carries_run_log_pointer(self, tmp_path: Path, state_file: Path):
        ...
        with (
            ...
            flow.read_state()
            cr = flow._run_lint_target(_target_handle("lite"))
    ...
    def test_previous_runs_log_is_erased_before_the_run(self, tmp_path: Path, state_file: Path):
        ...
        with (
            ...
            flow._run_lint_target(_target_handle("lite"))
    ...
    def test_hard_fail_pointer_omitted_when_log_write_failed(
        ...
        with (
            ...
            patch("booley.flows.lint.flow.write_run_log", side_effect=OSError("disk full")),
        ):
            mock_exec.return_value = MagicMock(
                returncode=2,
                stdout="",
...
class TestConfigDiscovery:
    def test_split_comma_separated(self, state_file: Path):
        ...
        assert tuple(target.selector for target in flow._get_targets()) == (
            "lite",
            "full",
            "combo",
        )
    ...
    def test_single_config(self, state_file: Path):
        ...
        assert tuple(target.selector for target in flow._get_targets()) == ("lite",)
