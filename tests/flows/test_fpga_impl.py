...
def test_negative_wns_fails_fpga_criterion(
    ...
    with (
        ...
        patch("booley.flows.fpga_edam.parse_fpga_reports", return_value=parsed),
        patch.object(
            flow,
            "_execute_boundary",
...
def test_enable_out_of_context_appends_synth_property(tmp_path: Path) -> None:
    ...
    from booley.flows import fpga_edam
...
def _patch_edam_build(captured: dict):
    ...
    return (
        patch("booley.flows.fpga_edam.build_fpga_edam", side_effect=fake_build_fpga_edam),
        patch("booley.flows.edam.configure", side_effect=fake_configure),
        patch("booley.flows.fpga_edam.fpga_run_command", return_value=["make", "-C", "x"]),
    )
...
class TestFpgaResolution:
    def test_resolution_forwards_sources_top_and_defines(
        ...
        with (
            ...
            cfg_p,
            run_p,
            patch("booley.flows.fpga_impl.validate_top_parameter_intent") as guard,
        ):
...
class TestFailureCapture:
    ...
    def test_route_not_reached_populates_capture_fields(
        ...
        with (
            ...
            patch(
                "booley.flows.fpga_edam.parse_fpga_reports",
