...
class TestHdlLoopFiring:
    ...
    def test_fires_once_per_test(
        ...
        with patch("booley.flows.sim.flow.subprocess.run", run_mock):
            flow = _make_flow(tmp_path, config="lite")
            result = flow._run()
...
class TestFiringIsVisible:
    ...
    def test_passing_run_records_each_firing(
        ...
        with patch("booley.flows.sim.flow.subprocess.run", MagicMock(return_value=_completed())):
            flow = _make_flow(tmp_path, config="lite")
            result = flow._run()
        assert result.exit_code == EXIT_SUCCESS
    ...
    def test_failing_hook_records_its_exit_status(
        ...
        with patch("booley.flows.sim.flow.subprocess.run", MagicMock(return_value=failed)):
            flow = _make_flow(tmp_path, config="lite")
            result = flow._run()
        assert result.exit_code == EXIT_FAILURE
