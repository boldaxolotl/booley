"""Architecture guards for shared low-level runtime mechanisms."""
...
def test_pid_liveness_consumers_import_the_runtime_module() -> None:
    offenders = []
    mcp_root = _SOURCE_ROOT / "mcp"
    for path in mcp_root.rglob("*.py"):
        tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
        for node in ast.walk(tree):
            if not isinstance(node, ast.ImportFrom):
                continue
            if node.module != "booley.ticket_board.helpers":
                continue
            if any(alias.name == "is_pid_alive" for alias in node.names):
                offenders.append(_relative(path, node.lineno))
