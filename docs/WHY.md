...
Every successful traced run ends with a fresh [FST](https://github.com/gtkwave/gtkwave) store that B-Wave has proven queryable. How the run reaches that store depends on the Target's existing trace recipe.

**An authored native-FST recipe stays native.** A Verilator Target that already declares `--trace-fst` has a matching C++ harness and trace options. Booley preserves that coherent setup, gives the binary a regular `.fst` destination, and lets B-Wave read the result directly. Rewriting half of that recipe would risk making the Verilator flags disagree with the project's harness, while converting its FST through VCD would add work without adding evidence.

**VCD is the portable fallback.** When a supported Target has no authored trace format, Booley adds VCD tracing. Verilator and Icarus can both produce VCD, so this remains the common path for projects without trace generation: the simulator writes VCD into a named pipe while `bwave build` converts it to FST. An explicitly authored VCD setup follows the same conversion path without having its options rewritten.
...
The VCD fallback still has a real cost: an encode and a decode. The FIFO trick takes the sting out: the simulator writes VCD into a named pipe, `bwave` converts in parallel, and the multi-GB intermediate never touches disk, so the remaining CPU overlaps with the run. Native FST avoids that conversion when the project already owns a coherent FST recipe.
