...
- `sim`: `--mode elab-only` compiles, elaborates, and links without running tests; `--mode elab-only-standalone` adds the stronger module sweep. Repeat `--test <name>` for exact registered names or use `--tests-file <path>`; configured skips apply only to default selection. Resume one durable ordinary-HDL Simulation Campaign with `--resume-from <exact-manifest.json>` (optionally with `--dry-run` to inspect it without execution). `--coverage` / `--cov` collects a native Coverage Campaign, and `--trace` captures waveforms for the simulation run. Focused Cocotb output summarizes unselected skips; pass `--result-verbosity full` to print every XML testcase entry (the complete XML and JSON artifacts are always retained)
