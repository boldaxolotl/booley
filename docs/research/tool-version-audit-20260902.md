# External program version audit - 2026-09-02

This audit covers every executable, compiler, runtime, immutable image, and
build or publication tool that `origin/main` at
`49c4e30e58097091a71d29ace26592c8c297dcd6` pins or invokes. Publisher
production channels were queried on 2026-09-02. Prereleases were excluded.

Six compatible publisher artifact updates appeared after the 2026-09-01 audit and are
routine updates in this refresh:

- Verible `v0.0-4157-gfdbac312` to `v0.0-4163-g6cce8f19`
- Claude Code `2.1.252` to `2.1.258`
- Codex CLI `0.151.0` to `0.152.1`
- Python `3.14.7-slim-bookworm` and `3.14.7-alpine3.24` rebuilt image indexes
- Docker CLI `29.7.2-cli` rebuilt image index

The image tag rebuilds retain the same language or CLI version and base
distribution. Their Linux/AMD64 manifests changed, so the immutable index pins
are refreshed and the affected images are rebuilt and tested.
...
| Verible | `v0.0-4157-gfdbac312` | `v0.0-4163-g6cce8f19` | [Verible release](https://github.com/chipsalliance/verible/releases/tag/v0.0-4163-g6cce8f19) | **routine update**: fixes formatter non-convergence for continuation comments and removes unused build dependencies without a CLI migration. Update the binary checksum, image assertion, tests, and supported-tool documentation together. |
...
| --- | --- | --- | --- | --- |
| Claude Code | `2.1.252` | `2.1.258` | [Claude Code v2.1.258](https://github.com/anthropics/claude-code/releases/tag/v2.1.258) | **routine update**: adds compatible agent, permission, and UI behavior and fixes session, sandbox, credential, MCP, and policy handling without a Booley configuration migration. Update the npm manifest, lockfile, image assertions, and tests together. |
| Codex CLI | `0.151.0` | `0.152.1` | [Codex 0.152.1](https://github.com/openai/codex/releases/tag/rust-v0.152.1) | **routine update**: adds compatible terminal, MCP, and app-server behavior and fixes approval, resume, Windows, and credential handling without a Booley configuration migration. Update the npm manifest, lockfile, image assertions, and tests together. |
| Node.js | `24.20.0` | `24.20.0` on the supported 24.x LTS lane | [Node release index](https://nodejs.org/dist/index.json) | **current** |
| Python sidecars | `3.14.7` on Bookworm and Alpine 3.24; indexes `sha256:416f0db2...` and `sha256:05b2b8b7...` | `3.14.7`; current indexes `sha256:9ab8d9c8...` and `sha256:c6ead215...` | [slim-bookworm tag](https://hub.docker.com/v2/repositories/library/python/tags/3.14.7-slim-bookworm), [Alpine tag](https://hub.docker.com/v2/repositories/library/python/tags/3.14.7-alpine3.24) | **routine update**: publisher rebuilds changed the Linux/AMD64 manifests while retaining Python and distribution versions. Refresh both immutable pins and rebuild all sidecars. |
| Python 3.13 sidecar controls | immutable `3.13.15` Bookworm and Alpine 3.24 indexes used only as the before-upgrade evidence baseline | same tags now resolve to rebuilt indexes `sha256:ed86c822...` and `sha256:7415fbc3...` | [slim-bookworm tag](https://hub.docker.com/v2/repositories/library/python/tags/3.13.15-slim-bookworm), [Alpine tag](https://hub.docker.com/v2/repositories/library/python/tags/3.13.15-alpine3.24) | **held**: these pins deliberately preserve the historical Python 3.13 control payload. Updating them would change the baseline rather than the shipped candidate images. |
| Rust/Cargo builder | `rust:1.98.0-slim-bookworm@sha256:1469a27c...` | Rust `1.98.0`; publisher tag digest matches | [Rust stable channel](https://static.rust-lang.org/dist/channel-rust-stable.toml), [official image](https://hub.docker.com/v2/repositories/library/rust/tags/1.98.0-slim-bookworm) | **current** |
...
| Docker CLI in reaper | `29.7.2`, index `sha256:000bb62f...`, Linux/AMD64 manifest `sha256:e650b7a5...` | `29.7.2`, index `sha256:3f474320...`, Linux/AMD64 manifest `sha256:eb952fbf...` | [Docker 29.7.2](https://github.com/moby/moby/releases/tag/docker-v29.7.2), [official image](https://hub.docker.com/v2/repositories/library/docker/tags/29.7.2-cli) | **routine update**: the publisher rebuilt the same CLI and Alpine lane; refresh the immutable pin and rebuild the reaper sidecar. |
