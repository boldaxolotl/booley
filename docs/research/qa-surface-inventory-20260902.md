...
against `main` at `5d3426eb7452ca09d2c3ba4f7333326138a5801a`. It inventories
...
| **Partial / prototype / planned** | Publicly described future or incomplete behavior. It is not a current regression contract. |
...
| --- | --- | --- |
| H-01 | Install the Python 3.11+ host CLI with `pipx` (preferred) or `pip`; `booley --version` identifies the installed release. Windows and Linux are supported; macOS is not ([installation](../../README.md#installation)). | Fresh install succeeds on a supported host, executable discovery works in a new shell, and version output matches the package under test. Capture the documented `pip` externally-managed failure/recovery only on an applicable distro. |
...
| H-06 | `booley session up/status/enter/down/validate/refresh` owns headless Session Runtime lifecycle. `enter -- <command>` preserves exit/signal semantics and cleans descendants; refresh is transactional and refuses to replace a VS Code-owned runtime ([headless runtime](../user/USAGE.md#entering-the-session-runtime-without-vs-code)). | Observe absent→running→stopped/absent, execute a success and nonzero command, interrupt one supervised process tree, validate the issued spec, and verify refresh either swaps to the expected immutable image or restores the old runtime. |
...
| P-02 | Every Booley Flow consumes an explicit named FuseSoC CAPI2 Target; there is no project-wide default. Qualified `vlnv#target` selectors resolve collisions. `booley targets`, filters, detail, JSON, and `booley_targets` expose the same surface ([Targets](../../src/booley/data/cheatsheet.md#targets)). | Use at least one multi-Target or colliding-name IP, compare CLI and MCP JSON identity, run a qualified selector, reject an ambiguous bare selector, and verify `.core` changes appear after refresh. |
...
| P-07 | Project Python dependencies are baked by `[sandbox].pip_requirements`; a custom Session Image can extend a managed image; host skills are opt-in and read-only; a post-setup hook is bounded, idempotent, and runs once per new Ticket Workspace ([sandbox dependencies](../user/CONFIG.md#sandbox), [advanced setup](../user/CONFIG.md#advanced-setups)). | Verify one derived image from pinned requirements, image freshness after a pin change, absence of runtime network install, read-only mounted host skill behavior if enabled, and a post-setup success plus controlled nonzero block. |
| P-08 | `booley-sandbox-riscv` is a supported Session Image variant with RISC-V GCC, `srec_cat`, `dtc`, Spike, and offline specifications ([RISC-V image](../user/CONFIG.md#risc-v-toolchain-image-booley-sandbox-riscv)). | If a CPU IP is chosen, build real firmware in pre-run commands, verify the exact toolchain and Spike identity inside the runtime, and retain the firmware/test artifact. Otherwise record this supported image lane as uncovered. |
...
| ID | Capability and contract | Minimum useful scenario evidence |
| --- | --- | --- |
| I-01 | Interactive Mode uses the Project's Session Runtime and provider-selected Claude or Codex client, with Booley MCP registration, skills, full in-container permission mode, default-deny network, and no Ticket/Criteria state ([Interactive Mode](../user/USAGE.md#interactive-mode), [architecture](../internals/ARCHITECTURE.md#interactive-mode)). | Across the suite, start one Claude and one Codex session, call `booley_status`, list Targets, run a real Flow and Specialist, inspect `.interactive_logs`, edit/commit in a disposable branch, and prove remote push is blocked. |
...
### Interactive client contract conflict
...
`booley` opens the configured CLI. Scenario authors must not silently pick one
interpretation. Resolve and document the supported Claude/Codex client matrix
before claiming client coverage; until then, exercise the shipped CLI path and
the applicable VS Code extension path as separate observations.
...
means at least one artifact-verified invocation of:
...
| ID | Capability and contract | Minimum useful scenario evidence |
| --- | --- | --- |
| S-01 | The active read-only `reviewer` has seven public focus contracts: RTL bugs, protocol, spec, code style, optimization, security, and TB quality. It reports CRITICAL/MAJOR/MINOR; `_done` is advisory terminal evidence, while `_clean` requires every finding fixed or explicitly waived and both stale after relevant edits ([reviewer catalog](../user/USAGE.md#reviewer)). | Distribute every focus across the initial scenarios. Seed at least one real finding and one clean focus, exercise `_done` and `_clean`, prove staleness after a source edit, and inspect report/transcript/waiver evidence. Spec focus must use the Ticket/spec path, not reconstructed chat prose. |
...
| W-03 | Public query/introspection commands are `list`, `signal`, `wave`, `value`, `find`, `sample`, `diff`, `distance`, `stats`, and `stuck`. They cover hierarchy, cycle/tick traces, snapshots, values/edges, triggered sampling, deltas, latency/period statistics, toggles/time-in-state, and stuck nets ([overview](../../crates/bwave/docs/public/commands/overview.md)). | Use one deterministic trace with known ground truth to invoke every command at least once, checking actual numeric/value results rather than rc. Include pattern miss, ambiguous signal, output limit, and unsupported JSON mode where documented. |
...
### B-Wave documentation conflicts

The bundled overview says every query/introspection command accepts
`--virtual`, while the virtual-signal reference excludes `list`, `stats`,
`stuck`, `diff`, and `build`; the `diff` page itself advertises `--virtual` in
its synopsis. The introduction says JSON exists only for `list`, `value`,
`find`, and `stats`, while several per-command synopses expose a generic format
flag and then say JSON is not implemented. Live per-command `--help` and actual
behavior must be reconciled before the scenario asserts those option matrices.
The command families themselves remain clearly supported.
...
| ID | Capability and contract | Minimum useful scenario evidence |
| --- | --- | --- |
| D-01 | `/booley-feedback` and `booley feedback` record defects, friction, impressions, and wins in an append-only Findings Log; entries have project/Booley/docs/unknown buckets, can be re-triaged and marked filed, and render one local unredacted report ([feedback config](../user/CONFIG.md#feedback-feedback)). | During every scenario, log findings at observation time plus wins as a denominator. Exercise all entry kinds, list, triage, filed exclusion, concurrent append safety, persistent report regeneration, and project-vs-Booley classification. |
...
The supported QA surface is the union of H-01–H-11, P-01–P-10, I-01–I-05,
F-01–F-04 and the mandatory EDA set, S-01–S-06, T-01–T-10, W-01–W-05,
...
behavior as mandatory coverage. Before finalizing client and detailed B-Wave
option assertions, resolve the two public documentation conflicts recorded
above.
