...
| S-04 | Supported built-in Criteria are per-Target `elab_pass_{target}`, global `elaborate_standalone`, per-Target `lint_clean_{target}`, all seven review families, per-Target `sim_pass_{target}`, per-test `cycle_count_{test}`, per-Target `mutation_score_{target}`, `synthesis_ok_{target}`, and `fpga_impl_ok_{target}`. A `coverage_{target}` template also exists but belongs to the hidden/incomplete coverage surface, not this supported set ([Criteria catalog](../user/USAGE.md#acceptance-criteria), [built-in templates](../../src/booley/data/criteria.toml)). | Every supported family needs at least one bound Ticket across the suite. Do not infer coverage from a standalone Flow that did not bind the sealed Criterion. |
