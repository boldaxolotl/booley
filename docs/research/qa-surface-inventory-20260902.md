...
| **Partial** | Only an explicitly named configuration or subset of the documented contract has shipped. Cover the supported subset; do not generalize it to missing configurations or behavior. |
...
| H-03 | `booley init` is host-only and idempotently creates or reconciles Project state, provider/auth policy, Ticket Board, Session Image, Git hooks, and devcontainer issuance. It supports `--check-only`, `--force`, `--seed`, `--skip-credentials`, explicit Claude/Codex auth selection, automatic Windows line-ending reconciliation, and the compatibility `--fix-line-endings` option ([Project Initialization](../user/SETUP.md#initialize-the-project--host), [line-ending recovery](../user/TROUBLESHOOTING.md#windows-first-run-problems)). | A port uses plain init, verifies the generated state and issued devcontainer, repeats init without drift, and proves host/container location guards. Across the suite, exercise both provider selections and credential policies. On Windows, also prove repository-local Git policy plus automatic LF normalization, compatibility-option acceptance, and the documented refusal of dirty, protected, or hard-linked files. |
...
| H-09 | License Profile registry CRUD and attachment to a Grant are **Supported** host-administration operations. The fixed FlexNet relay and real licensed checkout are **Experimental** because paid-seat checkout, accounting, concurrency, and return have not been validated ([Vivado policy](../user/SUPPORTED-EDA-TOOLS.md#vivado-host-provisioning-policy)). | Exercise profile registration/list/show/removal and Grant attachment as host administration. Keep relay validation in a separately qualified provisioned Linux x86-64 lane; real checkout cannot become mandatory until site approval and evidence exist. Report only the relay/runtime portion `not runnable` on Windows and unprovisioned hosts. |
...
| H-13 | Booley is Apache-2.0-licensed and local-first: orchestration, Project state, EDA work, and artifacts stay on the user's machine. Provider calls, an authorized license relay, notifications, and explicitly permitted egress are named exceptions rather than hidden hosted infrastructure ([feature catalog](../user/FEATURES.md#open-source-and-local-first), [security model](../internals/ARCHITECTURE.md#security--trust-model)). | Verify package/repository license metadata, local artifact/state locations, default-deny egress, and that each external route requires the documented provider, host-policy, or notification configuration. Do not interpret “local-first” as “no remote LLM provider call.” |
...
| ID | Capability and contract | Minimum useful scenario evidence |
| --- | --- | --- |
| P-01 | `/booley-setup` is plan-first Project Setup: port feasibility and one approved `SETUP-PLAN.md`, then environment, `.core`/`tests.toml`/`booley.toml`, Project guidance, plain Doctor, deep Doctor, optional native-flow parity, and mandatory findings/reporting. `/booley-setup new` is the post-scaffold greenfield route ([port sequence](../user/SETUP.md#porting-an-existing-project-plan-then-execute), [packaged setup skill](../../src/booley/data/skills/booley-setup/SKILL.md)). | The executor uses published Setup docs, skill instructions, `booley cheat`, and live help. Compare the approved plan to resulting files and record every deviation; exercise a native-flow comparison and Findings Log handoff, plus the `new` route in the scaffold scenario. Record the current selector mismatch—Setup advertises `0`–`5`, while the packaged skill accepts `0`–`6`—as a documentation Finding. No source-derived workaround may be consulted until an observation is captured. |
...
| ID | Capability and contract | Minimum useful scenario evidence |
| --- | --- | --- |
| I-01 | Interactive Mode uses the Project's Session Runtime and a Claude or Codex client, with Booley MCP registration, skills, full in-container permission mode, default-deny network, and no Ticket/Criteria state. The documented entry points include VS Code clients plus bare `booley`/`booley chat`, which launches the configured CLI ([Interactive Mode](../user/USAGE.md#interactive-mode), [architecture](../internals/ARCHITECTURE.md#interactive-mode)). | Across the suite, start one Claude and one Codex VS Code session as distinct coverage dimensions, call `booley_status`, list Targets, run a real Flow and Specialist, inspect `.interactive_logs`, edit/commit in a disposable branch, and prove remote push is blocked. Also smoke-test bare `booley`/`booley chat` for provider selection, Project working directory/session behavior, and actionable launch failure; this need not become another Cartesian client dimension. |
...
| F-02 | `lint` drives Verilator structural/semantic lint or Verible style/naming lint, supports scope filtering, deduplicates findings across Targets, and records findings even when warnings are configured not to fail the CLI. Waivers remain tool-native—Verilator `.vlt` and Verible configuration—rather than a Booley waiver layer ([Lint Flow](../user/FLOW_REFERENCE.md#lint), [feature catalog](../user/FEATURES.md#lint-triage)). | Use one Target for each linter; prove clean, warning, hard/infrastructure failure, scoped filtering, cross-Target dedupe, one native waiver for each linter, and the distinction between CLI rc and report `passed`. |
...
| S-04 | Supported built-in Criteria are per-Target `elab_pass_{target}`, global `elaborate_standalone`, per-Target `lint_clean_{target}`, all seven review families, per-Target `sim_pass_{target}`, per-test `cycle_count_{test}`, per-Target `mutation_score_{target}`, `synthesis_ok_{target}`, and `fpga_impl_ok_{target}`. Native `coverage_{target}` is also public after issue #213 phase 7, requires explicit `sim --coverage`, and remains independent of Simulation truth ([Criteria catalog](../user/USAGE.md#acceptance-criteria), [built-in templates](../../src/booley/data/criteria.toml)). | Every supported family needs at least one bound Ticket across the suite. Do not infer coverage from a standalone Flow that did not bind the sealed Criterion. |
...
### Reviewer-gate documentation conflict

S-01 follows the shipped `_clean` contract: every current finding, including
MINOR, must be fixed or explicitly waived. The feature catalog instead says
only unresolved CRITICAL findings prevent a Ticket from passing
([feature catalog](../user/FEATURES.md#multi-category-code-review),
[reviewer implementation](../../src/booley/specialists/reviewer.py)). Treat that
severity statement as a documentation Finding and do not weaken `_clean`
...
| T-04 | Review is a human decision, assisted by the public `/booley-ticket-triage` skill: small in-place correction then done, full reset to a clean queue, or archive. Ordinary review→queue is invalid. Reset retires branch/worktree and archives prior-run artifacts ([review decisions](../user/USAGE.md#ticket-board-lifecycle), [packaged triage skill](../../src/booley/data/skills/booley-ticket-triage/SKILL.md)). | Discover and invoke the triage skill, then use different Tickets for approval, reset, and archive; verify forbidden move rejection and prior-run preservation. Destructive reset/cleanup must target only disposable scenario Tickets. |
| T-05 | At the evaluated baseline, Scope is a commit-authorization boundary enforced by Ticket-worktree hooks; Harness bookkeeping and Target/control inputs are also hard boundaries. `submit_run_report` rejects staged, modified, deleted, or untracked work ([scope hook](../../src/booley/dev_support/scope_precommit_hook.py)). | Attempt one safe out-of-Scope commit and one bookkeeping edit, then verify hard rejection and clean recovery. Do not claim a supported Scope-deviation route: the public docs describe one, but no corresponding public bypass contract is identified. |
| T-06 | Acceptance Bases pin Criteria-bound Targets and repository refs at enqueue. Baseline-relative work uses immutable participant commits; a Target Plan classifies persistent, replacement, and ephemeral additions and derives the accepted Target surface ([Target threshold contract](../user/USAGE.md#threshold-parameters), [on-success](../user/USAGE.md#where-the-work-lands-on_success)). | Verify basis identity, reject a Developer-time input edit, revise through `return-to-draft`, and inspect destination history after one ephemeral comparison Target is removed. |
...
### Scope documentation conflict

The Usage guide and feature catalog say Scope is a plan rather than a fence and
that out-of-Scope authored files reach triage. The installed pre-commit hook
rejects every out-of-Scope staged file, while Harness policy code describes
ordinary authored deviations as advisory
([Scope](../user/USAGE.md#scope),
[feature catalog](../user/FEATURES.md#ticket-driven-workflow),
[scope policy](../../src/booley/harness/scope_policy.py),
[installed hook](../../src/booley/dev_support/scope_precommit_hook.py)). T-05
records the enforceable shipped hook behavior. The deviation promise is not a
Supported path until documentation and enforcement are reconciled.
...
[automatic Doctor](../../src/booley/harness/auto_doctor.py)). T-12 therefore
classifies only the overlapping blocked/Doctor subset as Supported and keeps
the completion probe visibly non-green. It must not promote the undocumented
events or selector to mandatory coverage.
...
| W-02 | The wrapper registers a file or sim directory under an alias and `_last`, can auto-build when requested, and persists named markers; aliases store replaceable absolute resolved paths plus registration time, not immutable or content-addressed artifact identities ([session registry](../../src/booley/bwave/sessions.py), [wrapper implementation](../../src/booley/bwave/cli.py)). | Register, restart the command/session context, query `@alias` and latest, replace an alias, set/list/delete markers, and prove a trace regenerated at the same path remains addressable with freshness based on its new mtime. Missing paths fail and old trace data warns; neither is an immutability check. |
| W-03 | Public query/introspection commands are `list`, `signal`, `wave`, `value`, `find`, `sample`, `diff`, `distance`, `stats`, and `stuck`. They cover hierarchy, cycle/tick traces, snapshots, values/edges, triggered sampling, deltas, latency/period statistics, toggles/time-in-state, and stuck nets. Meta commands `schema`, `docs`, and `skill` expose the machine grammar and bundled operational corpus. The wrapper also exposes `--version`/`-V` and documented legacy v0.1 invocation translation ([overview](../../crates/bwave/docs/public/commands/overview.md), [wrapper implementation](../../src/booley/bwave/cli.py)). | Use one deterministic trace with known ground truth to invoke every query/introspection command at least once, checking actual numeric/value results rather than rc. Include pattern miss, ambiguous signal, output limit, and each documented JSON-supported/unsupported boundary. Verify version output, one legacy translation, `schema`, topic list/search/show, and skill output are installed and internally coherent. |
...
`--virtual` and `--marker`, then its own consumer-options table narrows both
sets. The virtual-signal reference excludes `list`, `stats`, `stuck`, `diff`,
and `build`, while the `diff` page advertises `--virtual` in its synopsis. The
marker reference includes `signal` but excludes `diff`, while the overview
table excludes `signal` and the `diff` page advertises `--marker`. Current Rust
argument wiring supplies the consumer options to both commands. The
...
`--help` and behavior must resolve the `--virtual` and `--marker` disagreements
before a scenario freezes either option matrix. The command families remain
clearly supported.
...
| ID | Capability and contract | Minimum useful scenario evidence |
| --- | --- | --- |
| D-01 | `/booley-feedback` and `booley feedback` record defects, friction, impressions, and wins in a Findings Log. New entries use append-safe writes; triage and filed-state changes use controlled atomic whole-log rewrites. Entries have project/Booley/docs/unknown buckets and render one local unredacted report ([feedback config](../user/CONFIG.md#feedback-feedback), [Findings Log implementation](../../src/booley/feedback/findings.py)). | Exercise this end-user capability in one assigned probe: all entry kinds, list, triage, filed exclusion, concurrent append safety, atomic state rewrite, persistent report regeneration, and project-vs-Booley classification. It is not the scenario suite's own result channel; the map assigns that role to direct, unredacted QA records for Consolidate Findings. |
...
- `booley shell` is deliberately omitted from normal command discovery and is
  a diagnostic escape hatch, although Troubleshooting documents it. It is
  offline by default; `--net` is an explicit diagnostic override
  ([CLI implementation](../../src/booley/harness/booley.py),
  [Troubleshooting](../user/TROUBLESHOOTING.md#risc-v-firmware-wont-assemble-against-the-sandbox-gcc)).
...
- GitHub Issues as a Ticket Mode source: planned. The current Ticket Board is
  local Markdown only.
- Deterministic coverage measurement, including the built-in `coverage_{target}`
  Criterion template: hidden/incomplete.
...
Public scenario definitions may be versioned in the repository, but this is a
scenario-design constraint rather than a shipped feature. They must not leak
tested downstream identifiers, should pin upstream IP/commit/dependencies, and
must keep Findings and evidence separate from normative assertions. Each
capability row needs a scenario, step, stimulus, oracle, evidence pointer, and
status; an unexpected discovery failure remains failed even when a documented
fallback lets independent sections continue.
...
The supported QA surface is the union of H-01–H-13, P-01–P-10, I-01–I-05,
F-01–F-04 and the mandatory EDA set, S-01–S-06, T-01–T-12, W-01–W-05,
ST-01–ST-04, and D-01–D-03, subject to the stated mode/platform applicability
and only the blocked/Doctor subset of Partial T-12.
...
behavior as mandatory coverage. The map has settled primary client dimensions,
while I-01 retains a smoke check for the shipped bare CLI. Before finalizing
detailed B-Wave assertions, reconcile both virtual-signal and marker option
matrices. Ticket completion notification and the other named documentation/
implementation contradictions remain explicit non-green Findings rather than
