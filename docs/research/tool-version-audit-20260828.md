# External program version audit - 2026-08-28

This audit repeats the repository-wide search from the
[2026-08-27 audit](tool-version-audit-20260827.md) against `origin/main` at
`bd6fb5616100df06e05cfff62b21f16837404353`. It covers every executable,
compiler, runtime, immutable image, and build or publication tool that Booley
pins or invokes. Publisher production channels were queried on 2026-08-28;
prereleases were excluded.

Three stable releases appeared after the preceding audit and are routine
updates in this refresh:

- Claude Code `2.1.247` to `2.1.250`
- Ruff `0.16.4` to `0.16.5`
- PyPA build `1.5.0` to `1.6.0`

No EDA pin, compiler, sidecar image, or immutable image digest changed. The
OpenROAD, Verilator, Vivado, Node/Python/Ubuntu, and Spike holds remain
intentional and now link to concrete follow-up issues.

## EDA, simulation, synthesis, and lint tools

| Program | Installed pin | Current stable production release | Source | Outcome |
| --- | --- | --- | --- | --- |
| Yosys | `v0.68` / `38e001a6...` | `v0.68` | [Yosys v0.68](https://github.com/YosysHQ/yosys/releases/tag/v0.68) | **current** |
...
| Verible | `v0.0-4148-g1ea007ec` | `v0.0-4148-g1ea007ec` | [Verible release](https://github.com/chipsalliance/verible/releases/tag/v0.0-4148-g1ea007ec) | **current** |
...
| FuseSoC | `2.4.6` | `2.4.6` | [FuseSoC releases](https://github.com/olofk/fusesoc/releases/tag/2.4.6) | **current** |
| cocotb / `cocotb-config` | `2.0.1` | `2.0.1` | [cocotb v2.0.1](https://github.com/cocotb/cocotb/releases/tag/v2.0.1) | **current** |
...
| --- | --- | --- | --- | --- |
| Claude Code | `2.1.247` | `2.1.250` | [Claude Code v2.1.250](https://github.com/anthropics/claude-code/releases/tag/v2.1.250) | **routine update**: the patch release contains reliability fixes and has no migration notice. Update the npm manifest, lockfile, and image assertion together. |
| Codex CLI | `0.150.1` | `0.150.1`; `0.151.0-alpha.*` is prerelease | [Codex 0.150.1](https://github.com/openai/codex/releases/tag/rust-v0.150.1) | **current** |
| Node.js | `22.23.2` | `22.23.2` on the retained 22.x lane; newest LTS is `24.20.0` | [Node releases](https://nodejs.org/en/about/previous-releases) | **held**: the exact supported lane is current, while Node 24 requires agent-CLI policy and runtime compatibility testing. Follow-up: [#156](https://github.com/boldaxolotl/booley/issues/156). |
| Python sidecars | `3.13.15` on Bookworm and Alpine 3.24, exact digests | `3.13.15` on the retained lane; overall stable is `3.14.7` | [Python downloads](https://www.python.org/downloads/) | **held**: the exact 3.13 lane and digests are current; a 3.14 sidecar move is a runtime migration tracked by [#156](https://github.com/boldaxolotl/booley/issues/156). |
| Rust/Cargo builder | `rust:1.98.0-slim-bookworm@sha256:1469a27c...` | Rust `1.98.0`; tag digest matches | [Rust releases](https://blog.rust-lang.org/releases/) | **current** |
| Scheduled fuzz compiler | `nightly-2026-08-20` | nightly is not a stable release channel | [Rust toolchains](https://rust-lang.github.io/rustup/concepts/toolchains.html) | **held**: this immutable nightly is a tested fuzzing snapshot. A newer daily snapshot alone is not an actionable compatibility update; move it with a cargo-fuzz or LLVM requirement. |
| Docker CLI in reaper | `docker:29.7.2-cli@sha256:000bb62f...` | `29.7.2`; tag digest matches | [Docker 29.7.2 notes](https://docs.docker.com/engine/release-notes/29/#2972) | **current** |
...
| FlexNet relay image | `python:3.13.15-alpine3.24@sha256:540c7d91...` | exact tag digest matches | [official Python image](https://hub.docker.com/_/python) | **current** |
...
| --- | --- | --- | --- | --- |
| Ruff | `0.16.4` | `0.16.5` | [Ruff 0.16.5](https://github.com/astral-sh/ruff/releases/tag/0.16.5) | **routine update**: only preview-rule changes, bug fixes, server fixes, and documentation changes are listed. Update both exact project pins. |
...
| coverage.py | resolved through pytest-cov | `7.15.4` | [PyPI](https://pypi.org/project/coverage/) | **unversioned**: there is no direct repository pin. |
...
| PyPA build | workflow pin `1.5.0`; project lane `>=1.0` | `1.6.0` | [build 1.6.0](https://github.com/pypa/build/releases/tag/1.6.0) | **routine update**: the release corrects packaging and hash-verification behavior without a migration. Update all exact workflow pins. |
| Twine | `7.0.0` | `7.0.0` | [PyPI](https://pypi.org/project/twine/) | **current** |

The immutable GitHub Actions pins resolve to their current stable releases:
checkout `v7.0.1`, setup-python `v7.0.0`, upload-artifact `v7.0.1`,
download-artifact `v8.0.1`, setup-buildx `v4.3.0`, login `v4.6.0`,
build-push `v7.3.0`, rust-cache `v2.9.2`, and action-gh-release `v3.0.2`.
The pinned `pypa/gh-action-pypi-publish` commit matches its production
`release/v1` branch, and the pinned `dtolnay/rust-toolchain` commit matches its
production `stable` branch. Outcome: **current**.
