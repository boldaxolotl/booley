...
sealed Target Contract may retain a machine-owned canonical removal set, but
...
dependency; Booley pins and materializes the provider's sealed Target surface
internally, then refreshes and reseals the untouched consumer Ticket against
the accepted dependency state before its first execution. Many Tickets may
...
internal Target Contract mechanics and are omitted from normal user-facing
output.
