...
| D12 | Exact module `booley.targets.domain`; prefix `booley.fusesoc` | Respectively, prefix `booley.fusesoc`; exact modules `booley.targets.catalog`, `booley.targets.target`, and `booley.targets.target_surface` | Forbid | Target domain values stay dependency-neutral and FuseSoC adapters do not depend back on the Target catalog or its presentation facades. |
