...
[ADR 0059](../adr/0059-model-target-changes-with-ticket-target-plans.md).
...
uniquely at seal time. A Target may occur as an authored `target` once, a
...
may reference existing contract controls, or missing RTL/TB paths declared
Scope `[new]` where the existing Target Contract rules permit that. If sealing
would require other content, ticket creation reports a blocker.

The ticket creator opens the Ticket Workspace, applies the approved plan, and
`contract-seal` creates a new Target-contract commit before enqueue. A plan is
not satisfied merely because a matching Target already exists on the
destination branch: planned `target` entries must be definitions newly
authored by this Ticket.

### Seal invariants

At seal time, compare canonical Target declarations and owned test tables in
...
12. the sealed commits contain every validated authoring change.
...
The authored plan is the sole source for Target disposition. Sealing resolves
its selectors to canonical identities and derives the internal removal set:
...
Store the normalized Target Plan and derived removal set in the sealed Target
Contract. Acceptance continues to remove only the selected Target definition
...
from the sealed contract, never from `on_success`.
...
1. read the provider's sealed Target Contract and Target Plan;
2. materialize the exact provider-owned Target/control surface in the
   consumer's contract-authoring workspace;
...
4. pin the provider Ticket, exported canonical Target identity, exported role,
   and normalized surface identity in machine-owned contract data; and
...
5. rerun full Ticket and Target-contract validation;
6. create fresh sealed commits and update machine-owned contract metadata; and
...
blocks with `target-contract-change-required` and uses the existing contract
revision path.
...
### 2. Target-surface differ and seal validation
...
- Canonicalize the plan, derive removals, and include both in a new Target
  Contract schema revision.
- Keep internal removal data immutable after sealing.

Primary modules:

- `src/booley/ticket_board/contract_ops.py`
- `src/booley/ticket_board/target_contract.py`
- `src/booley/ticket_board/target_finalization.py`
- `src/booley/fusesoc/fusesoc_registry.py`

### 3. Acceptance cutover

- Make completion and the Acceptance Journal consume only the sealed
  contract's derived removal set.
...
- `src/booley/ticket_board/acceptance_journal.py`
- `src/booley/ticket_board/target_finalization.py`
...
- Add internal provider bindings to the Target Contract without adding a
  user-authored field or user-facing output.
- Materialize pinned provider surfaces for prospective seal validation.
...
- Verify provider content, reconstruct from current destinations, reapply local
  authoring changes, reseal, and fail closed on drift.
- Validate fan-out consumers, replacement chains, ambiguous providers, sibling
  replacement conflicts, provider reset/reseal, failure, and archive cases.
...
- Each role seals with exact canonical identities and derived removals.
...
- Owned tests tables seal; unplanned or ambiguously owned changes fail.
...
- A consumer can seal against a provider's persistent or replacement Target
...
- Successful dependency completion refreshes and reseals the consumer against
...
by a sealed Target Plan, acceptance applies only derived transitions, planned
