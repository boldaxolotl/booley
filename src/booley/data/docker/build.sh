...
COMMIT="$(PYTHONPATH="$BOOLEY_ROOT/src" "$PYBUILD" -P -c \
  'import sys; from pathlib import Path; from booley.runtime.build_stamp import write_build_stamp; print(write_build_stamp(Path(sys.argv[1]), include_development_context=False))' \
  "$BOOLEY_ROOT")"
