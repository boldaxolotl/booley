"""Human-facing labels, requirements, observations, and bounded snapshots.
...
_MAX_PRESENTATION_TEXT_BYTES = 2 * 1024
...
def _bounded_text(value: str) -> str:
    """Return a UTF-8-safe human-facing excerpt within the presentation budget."""
    raw = value.encode("utf-8")
    if len(raw) <= _MAX_PRESENTATION_TEXT_BYTES:
        return value
    suffix = "…"
    budget = _MAX_PRESENTATION_TEXT_BYTES - len(suffix.encode("utf-8"))
    return raw[:budget].decode("utf-8", errors="ignore") + suffix


def criteria_display_snapshot(state: DevelopmentState) -> dict[str, dict[str, Any]]:
    """Project durable Criteria into the compact representation consumed by the UI."""
    ...
    for key, entry in state.criteria.items():
        ...
        projected: dict[str, Any] = {
            ...
            "presentation": {
                "label": _bounded_text(presentation.label),
                "detail": _bounded_text(presentation.detail),
            },
        }
        if entry.stale:
            projected["stale"] = True
        if entry.ever_met:
            projected["ever_met"] = True
        if entry.ever_failed:
            projected["ever_failed"] = True
