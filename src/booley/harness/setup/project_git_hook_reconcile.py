...
def _publish_backup(path: Path, data: bytes, mode: int) -> Path:
    """Publish one legacy backup atomically."""
    target = _backup_candidate(path, data)
...
def _apply_adapters(adapters: list[_Adapter]) -> None:
...
def step_project_git_hooks(ctx: InitContext) -> None:
    ...
    try:
        ...
        _apply_adapters(adapters)
        _verify_adapters(adapters)
        errors = _cleanup_legacy(locations.project_dir / "hooks", current, previous)
        if errors:
