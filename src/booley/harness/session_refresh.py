...
_JOURNAL_PHASES = frozenset({"prepared", "parked", "image_selected", "issued", "verified"})
_JOURNAL_DIRECTIONS = frozenset({"restore_eligible", "committed_forward"})
...
def _require_mapping(raw: object, label: str) -> dict[str, Any]:
    if not isinstance(raw, dict) or any(not isinstance(key, str) for key in raw):
        raise sr.SessionError(f"Session refresh journal {label} must be an object")
    return raw
...
def _decode_snapshot(raw: object, project_root: Path) -> SessionSpecSnapshot:
    ...
    def content(name: str, present_name: str) -> bytes | None:
        present = values.get(present_name)
        encoded = values.get(name)
        if not isinstance(present, bool) or not isinstance(encoded, str):
            raise sr.SessionError("Session refresh journal snapshot fields are invalid")
        try:
    ...
    spec_mode = values.get("spec_mode")
    stamp_mode = values.get("stamp_mode")
    image_id = values.get("image_id")
    if (
        not isinstance(spec_mode, int)
        or isinstance(spec_mode, bool)
        or not isinstance(stamp_mode, int)
        or isinstance(stamp_mode, bool)
        or spec_mode < 0
        or spec_mode > 0o777
        or stamp_mode < 0
        or stamp_mode > 0o777
        ...
        raise sr.SessionError("Session refresh journal snapshot metadata is invalid")
    return SessionSpecSnapshot(
        dc.devcontainer_path(project_root),
        content("spec_content", "spec_present"),
        spec_mode,
        runtime_spec.stamp_path(project_root),
        content("stamp_content", "stamp_present"),
        stamp_mode,
        image_id,
    )
...
def _decode_issuance(raw: object, label: str) -> runtime_spec.Issuance:
    ...
    expected = frozenset(field.name for field in fields(runtime_spec.Issuance))
    _require_exact_fields(values, expected, label)
    try:
        issuance = runtime_spec.Issuance(**values)
    except TypeError as exc:
        raise sr.SessionError(f"Session refresh journal {label} is invalid") from exc
    if not isinstance(issuance.project_root, str) or not isinstance(issuance.image_id, str):
        raise sr.SessionError(f"Session refresh journal {label} is invalid")
    return issuance
...
def _decode_runtime(raw: object, project_root: Path) -> sr.ParkedSession | None:
    ...
    expected_backup = f"{expected_name}-pre-refresh"
    if values.get("name") != expected_name or values.get("backup") != expected_backup:
        raise sr.SessionError("Session refresh journal contains invalid Session Runtime names")
    try:
        parked = sr.ParkedSession(**values)
    except TypeError as exc:
        raise sr.SessionError("Session refresh journal prior runtime is invalid") from exc
    if (
        not isinstance(parked.was_running, bool)
        or not isinstance(parked.reconnect_egress, bool)
        or not parked.container_id
        or not parked.image_id
        or not parked.project_id
        or (parked.reconnect_egress and not parked.egress_network_id)
    ):
        raise sr.SessionError("Session refresh journal prior runtime identity is incomplete")
    return parked
...
def _decode_journal(raw: object, expected_root: Path) -> _RefreshJournal:
    ...
    if values.get("version") != _JOURNAL_VERSION or values.get("project_root") != str(
        expected_root
    ):
    ...
    if (
        ...
        or direction not in _JOURNAL_DIRECTIONS
    ):
        raise sr.SessionError("Session refresh journal replay metadata is invalid")
    target = values.get("target_image_id")
    payload = values.get("target_payload_fingerprint")
    if (target is not None and not isinstance(target, str)) or (
        payload is not None and not isinstance(payload, str)
    ):
        raise sr.SessionError("Session refresh journal replacement identity is invalid")
    replacement = values.get("replacement_issuance")
    prior_issuance = _decode_issuance(values.get("prior_issuance"), "prior issuance")
    if prior_issuance.project_root != str(expected_root):
        raise sr.SessionError("Session refresh journal prior issuance belongs to another Project")
    snapshot = _decode_snapshot(values.get("snapshot"), expected_root)
    if snapshot.image_id != prior_issuance.image_id:
        raise sr.SessionError("Session refresh journal predecessor identities disagree")
    parked = _decode_runtime(values.get("prior_runtime"), expected_root)
    if parked is not None and parked.image_id != prior_issuance.image_id:
    ...
    if replacement_issuance is not None and (
        replacement_issuance.project_root != str(expected_root)
        or replacement_issuance.image_id != target
    ):
        raise sr.SessionError("Session refresh journal replacement identities disagree")
    if direction == "committed_forward" and (
        phase != "verified" or replacement_issuance is None or target is None
    ):
...
def _resume_journal(journal: _RefreshJournal) -> RecoveryResult:
    ...
    if journal.direction != "committed_forward":
        journal = replace(journal, phase="verified", direction="committed_forward")
...
def recover_project_locked(project_root: Path) -> RecoveryResult:
    ...
    if journal.direction == "committed_forward" or _replacement_is_coherent(journal):
        return _resume_journal(journal)
    if journal.direction == "restore_eligible":
...
def _reconcile_refresh_image(
    ...
    journal = replace(journal, phase="parked")
    _write_journal(journal)
...
def _issue_and_verify_replacement(
    ...
    journal = replace(journal, phase="issued", replacement_issuance=issuance)
    _write_journal(journal)
...
def _recover_failed_refresh(project_root: Path, original: BaseException) -> None:
    try:
        ...
        if current.direction == "committed_forward":
