...
detailed_placement
estimate_parasitics -placement
{repair_timing_line}puts "BOOLEY_STAGE: sta_report"
report_design_area
...
def _resolve_openroad_run_inputs(
    ...
    Prints a warning and returns ``None`` for any missing prerequisite. The
    caller converts that into a failed physical synthesis run.
    """
...
def _print_openroad_area_markers(stdout: str) -> bool:
    """Print OpenROAD area markers and return whether design area was present.
...
def run_openroad_timing(
    ...
    Returns ``True`` only when timing and post-optimization area were both
    emitted. Missing prerequisites, a nonzero exit, or incomplete output makes
    physical synthesis fail; there is no logical-netlist timing fallback.
    """
    ...
    # emit_timing_markers recovers the effective period from the Target SDC
    # (ADR 0029 decision 6).
    surfaced = syn_core.emit_timing_markers(result.stdout, config, report_dir)
    area_surfaced = _print_openroad_area_markers(result.stdout)
    ...
    if not surfaced:
        ...
        return False
    if not area_surfaced:
        print("WARNING: OpenROAD completed but post-optimization area was not reported")
        return False
    return True
