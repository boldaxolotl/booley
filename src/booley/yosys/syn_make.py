...
* **configure** (:func:`configure_synthesis`) — renders everything up front:
  ...
  + ``stat`` emission), the OpenSTA/OpenROAD Tcl, the merged SDC, and a
  generated ``Makefile`` whose default target chains the stages
  (sv2v → yosys → sta) with each stage's stdout/stderr captured to a log file
...
def configure_synthesis(spec: SynthSpec, build_dir: Path) -> SynthPlan:
    ...
    Writes ``synth.ys`` (+ the sv2v recipe inside the Makefile),
    ``sta_constraints.sdc``, ``run_opensta.tcl`` (and ``run_openroad.tcl``
    when the engine is openroad), and the ``Makefile`` itself. Previous stage
    artifacts are cleared so the interpret half never reads a leftover
    ...
    if spec.timing.engine != "none":
        ...
        if spec.timing.engine == "openroad":
...
def _sta_fallback_sh(indent: str, *, continue_after: bool = False) -> list[str]:
    """Shell lines that locate and run OpenSTA, degrading to a WARNING.

    Missing binary or a nonzero exit must not fail the stage: timing never
    gates synthesis (legacy run_opensta parity) — the interpret half reports
    "timing unavailable" from the absent markers instead.
    """
    lines = [
        f"{indent}sta_bin=''; \\",
        f"{indent}if command -v sta >/dev/null 2>&1; then sta_bin=sta; \\",
        f"{indent}elif command -v opensta >/dev/null 2>&1; then sta_bin=opensta; fi; \\",
        f'{indent}if [ -n "$$sta_bin" ]; then \\',
        f'{indent}  "$$sta_bin" run_opensta.tcl > sta.log 2>&1 || '
        f"{{ tail -n {_FAIL_TAIL_LINES} sta.log; "
        "echo 'WARNING: OpenSTA failed; timing unavailable'; }; \\",
        f"{indent}else echo 'WARNING: OpenSTA requested but neither sta nor opensta"
        " is on PATH'; fi;",
    ]
    if continue_after:
        lines[-1] += " \\"
...
def _sta_recipe_lines(spec: SynthSpec) -> list[str]:
    """The timing stage's recipe body for the configured engine."""
    lines = ["\t@echo 'BOOLEY_STAGE: sta'"]
    if spec.timing.engine == "openroad":
        tech_lef = shlex.quote(str(openroad_timing.openroad_pdk_paths().tech_lef))
        body = [
            "\t@ok=0; \\",
            f"\tif command -v openroad >/dev/null 2>&1 && test -f {tech_lef}; then \\",
            "\t  if openroad -no_init -exit run_openroad.tcl > openroad.log 2>&1; then ok=1; \\",
            f"\t  else echo 'WARNING: OpenROAD failed; falling back to OpenSTA';"
            f" tail -n {_FAIL_TAIL_LINES} openroad.log; fi; \\",
            "\telse echo 'WARNING: OpenROAD unavailable in the Session Runtime;"
            " falling back to OpenSTA'; fi; \\",
            '\tif [ "$$ok" -eq 0 ]; then \\',
            *_sta_fallback_sh("\t  ", continue_after=True),
            "\tfi",
        ]
    else:
        fallback = _sta_fallback_sh("")
        body = ["\t@" + fallback[0], *[f"\t{line}" for line in fallback[1:]]]
    lines.extend(body)
    return lines
...
def _render_makefile(spec: SynthSpec, build_dir: Path) -> str:
    ...
    """
    engine = spec.timing.engine
    default_target = "sta" if engine != "none" else "yosys"
    ...
    lines = [
        ...
        "# PATH (sv2v/yosys + the configured timing engine); results land as",
...
def boundary_output(
    ...
    *,
    ...
    reg->reg / per-clock Fmax (``emit_timing_markers``), the OpenROAD
    pre-repair salvage and area markers, the ``STA_REPORT`` pointers, the
    false-pass log scan, and the sv2v source-provenance hint on failure.
...
def _timing_sections(
    ...
    """Timing-log sections + re-derived STA markers, engine-precedence aware.

    Mirrors the legacy engine selection: OpenROAD's markers win when it
    surfaced any timing (or its pre-repair snapshot could be salvaged,
    ADR 0029 D2); only then is the OpenSTA fallback log consulted. The
    marker-deriving helpers print — their output is captured here so it lands
    in the reconstructed report text instead of the EDA-tool process stdout.
    """
    spec = plan.spec
    report_dir = plan.build_dir / "reports" / "timing"
    parts: list[str] = []
    openroad_text = fresh_text("openroad.log") if spec.timing.engine == "openroad" else None
    sta_text = fresh_text("sta.log")
    ...
    with redirect_stdout(buf):
        ...
        if openroad_text is not None:
            ...
            if not surfaced and openroad_timing._timing_repair_enabled(spec.timing):
                surfaced = openroad_timing._salvage_pre_repair(
                    spec.timing, report_dir, openroad_text
                )
            openroad_timing._print_openroad_area_markers(openroad_text)
        if not surfaced and sta_text is not None:
            parts.append(f"--- sta.log ---\n{sta_text}")
            surfaced = syn_core.emit_timing_markers(sta_text, spec.timing, report_dir)
        if not surfaced and (openroad_text is not None or sta_text is not None):
