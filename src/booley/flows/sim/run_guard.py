...
The run-halves (:mod:`booley.flows.sim.backends.verilator` / :mod:`booley.flows.sim.backends.icarus`
/ :mod:`booley.flows.sim.backends.cocotb`) share this module so the guards live in one
...
def install_parent_death_guard() -> None:
    ...
    the ``python -m booley.flows.sim.backends.verilator`` supervisor underneath it and the
