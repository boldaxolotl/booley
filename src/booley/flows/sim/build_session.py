...
def project_compile_surface(project_root: Path) -> dict[str, str]:
    ...
    result: dict[str, str] = {}
    runtime_root = (resolve_checkout_project_dir(project_root) / ".runtime").resolve()
    try:
        for directory, children, names in os.walk(project_root, onerror=_raise_walk_error):
            ...
            children[:] = [
                ...
                if name != ".git"
                and (root / name).resolve() != runtime_root
            ]
            for name in names:
                path = root / name
...
def _has_generated_core_behavior(core_data: object) -> bool:
    if not isinstance(core_data, dict):
        return True
    if any(core_data.get(name) for name in ("generators", "generate", "scripts", "provider", "hooks")):
        return True
...
def _has_unsupported_icarus_recipe(prepared: PreparedSimulationBuild) -> bool:
    ...
    for line in scripts[0].read_text(encoding="utf-8").splitlines():
        ...
        if not source.is_file() or not source.resolve().is_relative_to(prepared.work_root.resolve()):
            return True
    makefile = root / "Makefile"
    commands = [line.strip() for line in makefile.read_text(encoding="utf-8").splitlines() if line.startswith("\t")]
    if len(commands) != 2:
        return True
    return not (
...
def _reuse_input_key(
    ...
    except (OSError, KeyError, UnicodeError, yaml.YAMLError, importlib.metadata.PackageNotFoundError):
        return None
...
def simulation_build_slot(handle: TargetHandle, variant: str = "") -> Path:
    ...
    return parent / f"{label[:60] or 'target'}--{digest}"
...
class SimulationBuildSession(AbstractContextManager["SimulationBuildSession"]):
    ...
    def __enter__(self) -> SimulationBuildSession:
        try:
            ...
            raise SimulationBuildSlotError(f"cannot lease Simulation slot {self.slot}: {exc}") from exc
    ...
    def new_generation(self) -> Path:
        ...
        try:
            ...
            if generations.is_symlink():
                raise SimulationBuildSlotError(f"generations directory is a symlink: {generations}")
            candidate = generations / secrets.token_hex(16)
            candidate.mkdir()
            return candidate
        except OSError as exc:
            raise SimulationBuildSlotError(f"cannot allocate Simulation generation: {exc}") from exc
    ...
    def try_reuse(self, candidate: PreparedSimulationBuild, key: str | None) -> PreparedSimulationBuild | None:
        ...
        try:
            ...
            selected = PreparedSimulationBuild(
            ...
            actual = {path.relative_to(selected.build_root).as_posix() for path in _runtime_artifacts(selected)}
            if actual != set(manifest["artifacts"]):
                self.cache_decision = "changed artifact"
                return None
            self.cache_decision = "hit"
            return selected
        except (OSError, KeyError, TypeError, ValueError, RuntimeError, SimulationBuildSlotError) as exc:
            self.cache_decision = (
                "missing provenance" if isinstance(exc, FileNotFoundError) else "malformed provenance"
            )
            return None
    ...
    def _hit_rejection(
        ...
        if manifest.get("schema") != 1 or manifest.get("target_identity") != candidate.target_identity:
            return (
                "schema mismatch"
        ...
        try:
            ...
            for name, digest in hashes.items():
                if _hash_file(root / name) != digest:
                    raise SimulationBuildSlotError(f"Simulation image changed before launch: {name}")
            if key is not None:
