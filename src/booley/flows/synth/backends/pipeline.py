...
* **configure** (:func:`configure_synthesis`) — renders everything up front:
  ...
  + ``stat`` emission), and, in physical mode, the OpenROAD Tcl that loads the
  Target-owned SDC files directly,
...
def configure_synthesis(spec: SynthSpec, build_dir: Path) -> SynthPlan:
    ...
    physical mode, ``run_openroad.tcl``. Previous
    ...
    """
    if runs_openroad(spec.timing.mode) and not spec.timing.sdc:
        raise BoundaryError(
            f"synth: physical Target {(spec.target or spec.design_name)!r} has no "
            "Target-owned SDC files"
        )
    ...
    if runs_openroad(spec.timing.mode):
        ...
        sdc_paths = tuple(Path(posix_relpath(path, build_dir)) for path in spec.timing.sdc)
        ...
        openroad_timing.write_openroad_script(
            ...
            build_dir,
            spec.timing,
            target=spec.target or spec.design_name,
