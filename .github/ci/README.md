...
duration. Raw progress is BuildKit's `rawjson` stream: one
`{"vertexes": [...]}` snapshot per line, merged per vertex digest. With the
default `docker` driver the image is written straight into the daemon store,
so the `exporting to image` vertex is the transfer/load phase and construction
ends where it starts; tarball-streaming drivers use their import vertices
instead. When no such vertex exists, transfer/load is explicitly
`unavailable`, the command duration remains attributed to the combined
construction/export/transfer/load operation, and the sample is not complete.
...
riscv_measurement=cold`). `baseline` keeps the default build path. `warm` restores a tooling-input- and stable-base-scoped local BuildKit
