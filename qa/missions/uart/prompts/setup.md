...
The Setup delegate runs `booley init --scaffold` with the selected provider, Verilator simulator/linter, SystemVerilog TB and ASIC support; then `/booley-setup new` inside the Sandbox. These choices are supplied in advance. Prove the generated counter and plain/deep Doctor baseline before replacing it.
