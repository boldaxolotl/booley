...
the run; note their exact versions and immutable Sandbox Image identity in `log.md`.
...
remaining time. The evaluator clips controls plus evaluation to a shared 30-minute phase
and each repair plus full rerun to 45 minutes; the operator stops starting new work
when the mission timebox runs out.
...
Keep failed control attempts alongside successful corrections under `evidence/`.

These fixtures exercise the named observation paths, not complete UART behavior.
...
the full case manifest and the actual candidate evaluation.

Depth/event-reset timeout cases keep paired stimuli, per-clock IRQ edges and
...
Invalid-address controls preserve occupied FIFO data and every stable CSR across
