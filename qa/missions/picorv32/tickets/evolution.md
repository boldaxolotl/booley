...
Render the fenced packet below: set `outer_destination_branch` and
`project_destination_ref` as for Ticket 1, and `configured_fpga_criterion` as the mission
says (the FPGA block with Vivado, empty without it). Change nothing else. Stage the result
under the ignored Project-data path `tmp/qa-inputs/<run-id>/ticket-2/packet.md` and keep a
copy in `evidence/`.
...
derives and authors the applicable Temporal Target definitions and their owned test tables;
this packet deliberately does not pre-author them.
