...
Render the fenced packet below: set `outer_destination_branch` to the outer branch
without `refs/heads/`, and `project_destination_ref` to the full nested ref. Change
nothing else. Stage the result under the ignored Project-data path
`tmp/qa-inputs/<run-id>/ticket-1/packet.md` and keep a copy in `evidence/`.
