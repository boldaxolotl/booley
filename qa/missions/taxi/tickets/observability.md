...
Used by the mission's `ticket1-observability` area. Submit the shared settings below plus the Ticket 1 section verbatim as the Ticket payload; Ticket Create must not infer omitted semantics:

- Codex form: `$booley-ticket-create --agent --no-confirm <Ticket payload from this file>`
- Claude-compatible form: `/booley-ticket-create --agent --no-confirm <Ticket payload from this file>`
