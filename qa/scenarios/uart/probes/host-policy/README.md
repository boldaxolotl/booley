...
`idle.toml` sets a 30-second idle limit; retain Session Runtime identity and actual expiry.
`cap.toml` sets one Session Runtime; open a second declared Project on the same isolated
...
A parser-only check cannot establish Session Runtime, session-cap, or egress behavior.
