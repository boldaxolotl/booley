...
This fixture supports `campaign.coverage-aggregate-resume`,
`campaign.acceptance-recovery`, and `campaign.corrupt-terminal-rejection`.
All three Checks are pending until a separately authorized
`coverage-lifecycle-ubuntu-codex-cli` Scenario Run records product evidence.
Unit or fixture validation does not complete a Check.
...
Ledger every run-owned report root, nested Coverage Campaign, Ticket state,
Acceptance Journal directory, temporary-path fault, corrupt copy, process group,
and fixture registration before the first mutation. Recovery always archives
the observed failure before restoring only owned bytes or paths, then runs the
valid control in the same Scenario Run. It never depends on the negative Check
passing.
...
registration. Preserve the interrupted, failed, corrupt, restored, and selected
evidence archives. Prove the shared fixture and pinned sources are unchanged and
record every resource disposition in the Scenario cleanup ledger. The existing
480-minute deadline is unchanged: these Checks reuse the terminal aggregate and
the exercise-phase contingency; if a fresh run cannot reach cleanup by minute
450, leave missing Checks incomplete rather than extending the deadline.
