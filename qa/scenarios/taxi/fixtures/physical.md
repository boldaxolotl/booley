...
The whole run remains eight hours. Physical work runs within the existing approved baseline, Ticket2 and final-regression phase ceilings (with the Project Setup addition inside its Project Setup ceiling); no Session Image preparation, baseline or comparison is moved outside the clock, and no old artifact supplies a fresh required pass. Time exhaustion blocks missing physical work and makes qualification incomplete unless a trustworthy failure already makes it failed.
...
| `taxi-10g-mac-port-evolution.physical.recipe-identity` | Core | Baseline/candidate/final use same selected library/technology and physical recipe. | Exact Session Image, EDA tool, library, and recipe identities and pair comparison. |
