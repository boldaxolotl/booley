...
def spike_elf(path: Path, ram_start: int, ram_end: int) -> dict:
    ...
    _require(
        0 < phnum <= 64 and phoff + phnum * phentsize <= len(data),
    ...
    for index in range(phnum):
        ...
        if elf64:
            kind, _, _, vaddr, paddr, _, memsz, _ = struct.unpack_from(
                endian + "IIQQQQQQ", data, offset
            )
        else:
            kind, _, vaddr, paddr, _, memsz, _, _ = struct.unpack_from(
        ...
        _require(
            memsz > 0 and ram_start <= address and address + memsz <= ram_end,
...
def _warning_keys(report: dict) -> list[tuple]:
    ...
    for value in warnings:
        ...
        _require(
            all(field in item for field in ("rule", "file", "line", "message")),
...
def _target_ids(report: dict, name: str) -> set[str]:
    ...
    _require(
        isinstance(target_results, list)
        and all(
            isinstance(row, dict) and isinstance(row.get("target"), str) and bool(row["target"])
...
def lint_dedupe(first: dict, second: dict, combined: dict) -> dict:
    ...
    first_keys, second_keys, combined_keys = map(
        set, map(_warning_keys, (first, second, combined))
    )
    ...
    _require(
        len(combined.get("warnings", [])) == len(combined_keys),
...
def lint_waiver(
    before: dict, after: dict, intended_rule: str, control_rule: str | None = None
) -> dict:
    ...
    intended = Counter({key: count for key, count in earlier.items() if key[0] == intended_rule})
    _require(bool(intended), "intended warning absent before waiver")
    _require(
        sum(intended.values()) == 1, "waiver stimulus must contain exactly one intended warning"
    )
    if control_rule is not None:
        controls = Counter(
            {key: count for key, count in earlier.items() if key[0] == control_rule}
        )
        _require(
            bool(controls) and all(later[key] == count for key, count in controls.items()),
            "unrelated control warning did not survive waiver",
        )
    _require(
        all(key[0] != intended_rule for key in later), "intended warning remains after waiver"
    )
...
def _synth_result(report: dict, target: str) -> tuple[dict, dict]:
    ...
    _require(
        comparison.get("basis_valid") is True and comparison.get("basis_errors") == [],
...
def _synth_revisions(result: dict, comparison: dict) -> tuple[str | None, str | None]:
    ...
    baseline_producer = _mapping(
        baseline_provenance.get("producer"), "synthesis baseline producer"
    )
...
def _synth_expectations(summary: dict, expected: dict) -> tuple[dict, str, str]:
    for field in (
        ...
        _require(
            isinstance(value, str) and bool(value.strip()), f"synthesis comparison lacks {field}"
        )
    ...
    _require(
        ...
        and bool(ref.strip())
        and re.fullmatch(r"[0-9a-f]{40}", baseline_ref) is not None
    ...
    for field in ("delta_pct", "timing_delta_pct"):
        ...
        _require(
            type(expected_value) in (int, float)
...
def synth_baseline(summary: dict, report: dict, expected: dict) -> dict:
    ...
    _require(
        type(summary.get("flow_exit")) is int and summary["flow_exit"] == 0,
        "synthesis comparison did not exit successfully",
    )
    _require(
        summary.get("infra_error") in (None, ""),
    ...
    _require(
        candidate_revision == expected_candidate_ref,
        "synthesis candidate revision differs from declared commit",
    )
    _require(
        baseline_revision == expected_ref,
    ...
    _require(
        identity.get("target_identity") == summary["candidate_identity"]
    ...
    _require(
        area.get("delta_pct") == summary["delta_pct"]
...
def lint_command(argv: list[str]) -> dict:
    ...
    _require(
        argv[:4] == ["booley", "session", "enter", "--"], "expected booley session enter -- ..."
    )
...
def distance_rows(text: str, expected_distances: list[int]) -> dict:
    ...
    _require(
        all(int(end) - int(start) == int(delta) for start, end, delta in rows),
...
def required_subjects(pdf_text: dict[str, str], subjects: list[str]) -> dict:
    ...
    _require(
        all(isinstance(value, str) for value in pdf_text.values())
...
def vivado_implementation(report: dict, artifacts: dict, before: dict, retained_dir: Path) -> dict:
    ...
    _require(
        type(report.get("exit_code")) is int and report["exit_code"] == 0,
    ...
    _require(
        implementation.get("grade") == "pass" and implementation.get("passed") is True,
    ...
    _require(
        bool(cache)
        and all(isinstance(item, dict) and item.get("cached") is False for item in cache.values()),
    ...
    for name in required:
        ...
        _require(
            ...
            and current.get("size", 0) > 0
            and current.get("sha256")
            and current.get("mtime_ns"),
            f"missing declared routed artifact: {name}",
        )
        prior = before.get(name)
        _require(
            prior is None or isinstance(prior, dict), f"invalid prior routed artifact: {name}"
        )
        _require(
            prior is None or current["mtime_ns"] > prior.get("mtime_ns", 0),
            f"routed artifact is not fresh: {name}",
        )
        retained = retained_dir / name
        _require(
            retained.is_file() and retained.stat().st_size == current["size"],
            f"retained routed artifact missing or changed: {name}",
        )
        _require(
            hashlib.sha256(retained.read_bytes()).hexdigest() == current["sha256"],
...
def stealth_native(paths: list[str]) -> dict:
    ...
    _require(
        isinstance(paths, list) and all(isinstance(path, str) and bool(path) for path in paths),
        "projected paths must be a list of nonempty strings",
    )
    native = [path for path in paths if all(not part.startswith(".") for part in Path(path).parts)]
...
def same_bwave_mode(child: dict, replay: dict) -> dict:
    ...
    for record, name in ((child, "child"), (replay, "replay")):
        _require(all(field in record for field in fields), f"B-Wave {name} query is incomplete")
        _require(
            isinstance(record["trace"], str) and bool(record["trace"]),
        ...
        for field in ("clock", "reset"):
            _require(
                record[field] is None or (isinstance(record[field], str) and bool(record[field])),
                f"B-Wave {name} {field} is invalid",
            )
        _require(
            isinstance(record["sampling"], str) and bool(record["sampling"]),
            f"B-Wave {name} sampling is invalid",
        )
    _require(
        all(child.get(field) == replay.get(field) for field in fields),
...
def verdict(expected: str, observed: str) -> dict:
    ...
    return {"declared_expected": expected, "observed": observed, "matches": expected == observed}
...
def oracle(record: dict) -> dict:
    ...
    if kind == "lint-dedupe":
        ...
        result = lint_waiver(
            record["before"], record["after"], record["intended_rule"], record.get("control_rule")
        )
        ...
        result = vivado_implementation(
            record["report"], record["artifacts"], record["before"], Path(record["retained_dir"])
        )
