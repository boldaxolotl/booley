...
def _owner(record: dict) -> dict:
    ...
    _need(
        owner["registration"] in registrations,
...
def _identity(snapshot: dict, owner: dict, name: str) -> None:
    _need(
        snapshot.get("project_root") == owner["project_root"],
...
def _protected_unchanged(record: dict) -> None:
    ...
    _need(
        set(before) == {"grants", "installations"} and set(after) == {"grants", "installations"},
        "borrowed inventory must contain grants and installations",
    )
    for kind in ("grants", "installations"):
        _need(
            isinstance(before[kind], list) and isinstance(after[kind], list),
            f"borrowed {kind} inventory must be a list",
        )
        _need(
            all(isinstance(item, dict) for item in before[kind] + after[kind]),
            f"borrowed {kind} inventory rows must be mappings",
        )
        old = Counter(
            json.dumps(item, sort_keys=True, separators=(",", ":")) for item in before[kind]
        )
        new = Counter(
            json.dumps(item, sort_keys=True, separators=(",", ":")) for item in after[kind]
        )
        _need(old == new, f"borrowed {kind} changed during run-owned transition")


def _session_transition(
    before: dict, revoked: dict, regranted: dict, issued: dict, started: dict
) -> None:
    ...
    _need(
        before_grant_epoch < revoked_grant_epoch < regranted_grant_epoch,
        "Grant epochs do not prove revoke then regrant",
    )
    _need(
        _epoch(revoked, "session_grant_epoch", "revoked") == before_session_epoch
    ...
    for name, snapshot in (("revoked", revoked), ("regranted", regranted)):
        _need(
            snapshot.get("session_valid") is False
            and snapshot.get("session_running") is False
            and snapshot.get("mount_probe") is False,
            f"{name}: stale Session was not invalidated",
        )
    _need(
        _epoch(issued, "grant_epoch", "issued") == regranted_grant_epoch
        and issued.get("session_running") is False
        and issued.get("mount_probe") is False,
        "issued Session does not belong to the regranted epoch",
    )
    _need(
        _epoch(issued, "session_grant_epoch", "issued") == regranted_grant_epoch
        and issued.get("session_valid") is True,
        "stale Session spec; run booley init --seed after regrant",
    )
    _need(
        _epoch(started, "session_grant_epoch", "started")
        == _epoch(started, "grant_epoch", "started")
        == regranted_grant_epoch
        and started.get("session_valid") is True
        and started.get("session_running") is True,
...
def grant_replacement(record: dict) -> dict:
    ...
    _need(
        isinstance(old, str) and old in owner["run_owned_registrations"],
        "existing Grant is borrowed; do not revoke it",
    )
    _need(revoked.get("grant_registration") is None, "old run-owned Grant was not revoked")
    _need(
        regranted.get("grant_registration") == owner["registration"],
        "new run-owned Grant is missing",
    )
    _need(
        issued.get("grant_registration") == owner["registration"],
        "Grant changed during host Session issuance",
    )
    _need(
        started.get("grant_registration") == owner["registration"],
        "Grant changed before Session start",
    )
    _session_transition(before, revoked, regranted, issued, started)
    _need(
        started.get("mount_probe") is True, "intended Vivado mount/command probe did not succeed"
    )
...
def ready(record: dict, name: str = "current") -> dict:
    ...
    _need(
        current.get("grant_registration") == owner["registration"],
        "run-owned Vivado Grant is missing",
    )
    _need(
        _epoch(current, "session_grant_epoch", name) == _epoch(current, "grant_epoch", name)
...
def denied(record: dict) -> dict:
    ...
    _need(
        result.get("flow_executed") is False, "revoked-Grant denial did not prevent FPGA execution"
    )
    _need(
        isinstance(owner.get("evidence_root"), str) and bool(owner["evidence_root"]),
    ...
    _need(
        evidence_root.is_dir() and log_path.is_file() and log_path.is_relative_to(evidence_root),
        "denial log is outside the retained evidence root",
    )
    content = log_path.read_bytes()
    _need(
        hashlib.sha256(content).hexdigest() == result.get("log_sha256"),
        "denial log digest does not match retained evidence",
    )
    text = content.decode("utf-8")
    _need(re.search(r"(?m)^exit:\s*2\s*$", text) is not None, "denial log does not record exit 2")
    _need(
        '"flow", "fpga"' in text or re.search(r"\bbooley\s+flow\s+fpga\b", text),
        "denial log does not identify the FPGA command",
    )
    authority_denials = (
        "host-issued spec stamp",
        "no exact vivado grant",
        "has no exact vivado grant",
    )
    _need(
        any(fragment in text.casefold() for fragment in authority_denials),
...
def main() -> None:
    ...
    try:
        ...
        result = {"replace": grant_replacement, "ready": ready, "denied": denied}[args.stage](
            record
        )
