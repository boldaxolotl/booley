...
The repair must replace the injected AND expression with the canonical reduction-OR form `assign we = |mem_wstrb;`. The fixture retains a separate negative case for restoring the exact clean `HEAD` bytes: that no-op repair must be rejected because it cannot produce the required meaningful local commit diff.
